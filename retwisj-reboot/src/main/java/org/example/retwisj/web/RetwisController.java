...
import org.example.retwisj.Post;
import org.example.retwisj.Range;
import org.example.retwisj.RetwisSecurity;
import org.example.retwisj.redis.RetwisRepository;
...
public class RetwisController {
	...
	public String posts(@PathVariable String name, @RequestParam(required = false) String replyto, @RequestParam(
			required = false) String replypid, @RequestParam(required = false) Integer page, Model model) {
		checkUser(name);
		...
		model.addAttribute(
				"moreposts",
				(RetwisSecurity.isUserSignedIn(targetUid) ? retwis.hasMoreTimeline(targetUid, range) : retwis.hasMorePosts(
						targetUid, range)));
